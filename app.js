...
window.saveNewPassword = async () => {
  ...
  try {
    ...
    localStorage.setItem('ienomics_familyCode', code);
    applyFuriganaState();
    await showParentSetupComplete(code);
    runMigrationAndLoadChildren(user.uid);
    // 子供の新規連携と同じく、初回ガイド終了後に通知許可を案内する
    if (!hasSeenTutorial()) {
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('sw.js?v=264').catch(err => console.warn('SW登録失敗:', err));
  });
}